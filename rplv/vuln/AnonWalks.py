...
class AnonymousWalks(object):
    ...
    def create_random_walk_graph(self, bias=None, densities=None):
        ...
        for node in self.graph:
            ...
            # base weight (edge weight or 1) * bias factor(u->v)
            weighted = {
                v: edges[v].get(label_name, 1.0) * self._bias_factor(node, v, bias, densities)
            ...
            for v, w in weighted.items():
                RW.add_edge(node, v, weight=w / total)
        self.rw_graph = RW
        # Invalidate any cached transition samplers (RW graph depends on weights/bias)
        self._rw_nbrs = None
        self._rw_cdfs = None
        self._rw_sampler_ready = False

    
    def _prepare_rw_samplers(self):
        '''Precompute per-node neighbor arrays and CDFs for fast random-step sampling.

        After calling this, `_random_step_node` can sample the next neighbor in O(log deg)
        using `np.searchsorted` instead of scanning all outgoing edges.
        '''
        ...
        nbrs = {}
        cdfs = {}
        for u in self.rw_graph.nodes():
            vs = list(self.rw_graph[u])
            if not vs:
                continue
            probs = np.fromiter((self.rw_graph[u][v]['weight'] for v in vs), dtype=float, count=len(vs))
            total = float(probs.sum())
            if total <= 0:
                continue
            probs /= total
            cdf = np.cumsum(probs)
            cdf[-1] = 1.0  # ensure proper coverage
            nbrs[u] = np.asarray(vs, dtype=object)
            cdfs[u] = cdf

        self._rw_nbrs = nbrs
        self._rw_cdfs = cdfs
        self._rw_sampler_ready = True
    ...
    def _random_step_node(self, node):
        ...
        Fast-path: if `_prepare_rw_samplers()` was called, sampling uses a cached per-node CDF
        for O(log deg) selection instead of scanning all outgoing edges.
        '''
        if self.rw_graph is None:
            raise ValueError("Create a Random Walk graph first with {}".format(self.create_random_walk_graph.__name__))

        # Fast path using cached neighbor arrays + CDFs (built by _prepare_rw_samplers)
        if getattr(self, '_rw_sampler_ready', False) and getattr(self, '_rw_nbrs', None) is not None and node in self._rw_nbrs:
            nbrs = self._rw_nbrs[node]
            cdf = self._rw_cdfs[node]
            r = random.random()
            j = int(np.searchsorted(cdf, r, side='right'))
            if j >= len(nbrs):  # guard against numerical edge cases
                j = len(nbrs) - 1
            return nbrs[j]

        # Fallback: linear scan over outgoing edges (original behavior)
    ...
    def embed(self, steps, method = 'exact', MC = None, delta = 0.1, eps = 0.1,
        ...
        # remember config and build transition graph with bias
        self.bias = bias
        self.densities = densities
        self.create_random_walk_graph(bias=bias, densities=densities)
...
#######################################################
    ...
    def _out_degree(self, v):
        # Use out-degree on DiGraph, degree on Graph
        return self.graph.out_degree(v) if isinstance(self.graph, nx.DiGraph) else self.graph.degree(v)

    def _bias_factor(self, u, v, bias, densities):
        """
        Multiplicative bias for transition u->v.
        Returns a non-negative factor; caller will normalize across v.
        """
        if bias in (None, 'none'):
            return 1.0
        if bias == 'high_degree':
            return float(max(self._out_degree(v), 0))
        if bias == 'low_degree':
            d = float(self._out_degree(v))
            return 1.0 / max(d, 1.0)  # avoid div-by-0
        if bias == 'high_density':
            if densities is None:
                return 1.0
            val = float(densities.get(v, 0.0))
            return max(val, 0.0)
        if bias == 'low_density':
            if densities is None:
                return 1.0
            val = float(densities.get(v, 0.0))
            return 1.0 / max(val, 1e-12)  # avoid div-by-0
        raise ValueError("Unknown bias: {}".format(bias))




    
    def node_aw_embeddings_sampling(self, steps, MC=None, delta=0.1, eps=0.1,
                                   labels=None, keep_last=True, normalize=True, verbose=False,
                                   bias='none', densities=None, precompute_sampler=True):
        """Per-node anonymous-walk distributions via Monte-Carlo sampling.
        ...
        MC : int or None
            Number of sampled walks *per node*. If None, uses n_samples(steps, delta, eps).
        delta, eps : float
            Used only when MC is None. Same meaning as in embed(): confidence/accuracy knobs.
        labels : {None, 'edges', 'nodes', 'edges_nodes'}
            Which label scheme to use, consistent with embed().
        keep_last : bool
            Passed to _all_paths* to build the basis.
        normalize : bool
            If True, normalize counts to sum to 1.
        bias, densities :
            Passed through to create_random_walk_graph().
        precompute_sampler : bool
            If True, precomputes per-node CDF samplers to speed up _random_step_node.

        Returns
        -------
        node2vec : dict
            {node: tuple(float)} aligned with meta['meta-paths'].
        meta : dict
            {'meta-paths': list_of_patterns} giving the basis ordering.
        """
        # Build transition graph (respecting bias)
        self.bias = bias
        self.densities = densities
        self.create_random_walk_graph(bias=bias, densities=densities)

        # Build the canonical basis of patterns for this steps/labels
        if labels is None:
            self._all_paths(steps, keep_last=keep_last)
        elif labels == 'edges':
            self._all_paths_edges(steps, keep_last=keep_last)
        elif labels == 'nodes':
            self._all_paths_nodes(steps, keep_last=keep_last)
        elif labels == 'edges_nodes':
            self._all_paths_edges_nodes(steps, keep_last=keep_last)
        else:
            raise ValueError("labels must be one of: None, 'edges', 'nodes', 'edges_nodes'.")

        basis = [tuple(p) for p in self.paths[steps]]
        K = len(basis)
        walk_ids = {p: i for i, p in enumerate(basis)}

        # Default MC from the concentration bound used elsewhere in the class
        if MC is None:
            MC = self.n_samples(steps, delta, eps)
            if verbose:
                print("Using MC = {} samples per node (delta={}, eps={})".format(MC, delta, eps))

        if precompute_sampler:
            self._prepare_rw_samplers()

        node2vec = {}
        zeros = tuple([0.0] * K)

        # Local bindings for speed
        aw = self._anonymous_walk
        get_id = walk_ids.get

        for s in self.graph.nodes():
            # Nodes with no outgoing transitions: return zeros
            if (s not in self.rw_graph) or (len(self.rw_graph[s]) == 0):
                node2vec[s] = zeros
                continue

            ids = np.empty(MC, dtype=np.int32)
            for i in range(MC):
                pat = aw(s, steps, labels)
                ids[i] = get_id(pat, -1)

            # Defensive: basis should cover all generated patterns, but filter if needed
            if (ids < 0).any():
                ids = ids[ids >= 0]
                if ids.size == 0:
                    node2vec[s] = zeros
            ...
            vec = np.bincount(ids, minlength=K).astype(np.float64)
            if normalize and vec.sum() > 0:
                vec /= vec.sum()

            node2vec[s] = tuple(vec.tolist())

        return node2vec, {'meta-paths': basis}

    def node_aw_embeddings(self, steps, labels=None, keep_last=True, normalize=True, verbose=False,
                          bias='none', densities=None):
        """
        Per-node anonymous-walk distributions.

        Returns:
            node2vec: dict {node: (p1, p2, ..., p_k)}, where k = # of AW patterns of length `steps`
            meta: {'meta-paths': self.paths[steps]} listing the AW pattern order
        """
        # 1) Build RW graph (edge transition probs) and the canonical pattern list for this `steps`.
        #self.create_random_walk_graph()
        self.bias = bias
        self.densities = densities
        self.create_random_walk_graph(bias=bias, densities=densities)
        ...
        if labels is None:
            self._all_paths(steps, keep_last=keep_last)
            _pattern = self.walk2pattern
        elif labels == 'edges':
            self._all_paths_edges(steps, keep_last=keep_last)
            _pattern = self.walk2pattern_edges
        elif labels == 'nodes':
            self._all_paths_nodes(steps, keep_last=keep_last)
            _pattern = self.walk2pattern_nodes
        elif labels == 'edges_nodes':
            self._all_paths_edges_nodes(steps, keep_last=keep_last)
            _pattern = self.walk2pattern_edges_nodes
        else:
            raise ValueError("labels must be one of: None, 'edges', 'nodes', 'edges_nodes'.")

        patterns_list = self.paths[steps]  # canonical ordering (defines p1..pk)

        node2vec = {}

        # 2) For each node, enumerate all length-`steps` walks and accumulate probability by AW pattern.
        # for s in self.rw_graph.nodes():
        for s in self.graph.nodes():
            counts = defaultdict(float)

            # If the node has no outgoing edges and steps>0 → all-zero vector.
            if steps == 0:
                # Length-0 "walk": just the node itself; map it to a 1-hot if desired.
                # Here we keep a zero vector since usually steps>=1 is used.
                vec = [0.0] * len(patterns_list)
                node2vec[s] = tuple(vec)
                continue

            #if len(self.rw_graph[s]) == 0:
            #    node2vec[s] = tuple([0.0] * len(patterns_list))
            #    continue
            # If the node is missing from rw_graph or has no out-neighbors → zeros
            if (s not in self.rw_graph) or (len(self.rw_graph[s]) == 0):
                node2vec[s] = tuple([0.0] * len(patterns_list))
                continue

            def dfs(u, depth, walk, prob):
                if depth == steps:
                    pat = _pattern(walk)  # uses existing walk2pattern* helpers
                    counts[pat] += prob
                    return
                for v in self.rw_graph[u]:
                    p = self.rw_graph[u][v]['weight']  # transition prob
                    dfs(v, depth + 1, walk + [v], prob * p)

            dfs(s, 0, [s], 1.0)

            # Turn the pattern dict into a vector aligned with `patterns_list`
            vec = [counts.get(tuple(p), 0.0) for p in patterns_list]

            # Each node's probabilities should sum to 1 (for reachable walks); normalize just in case.
            if normalize:
                total = sum(vec)
                if total > 0:
        ...
        return node2vec, {'meta-paths': patterns_list}
